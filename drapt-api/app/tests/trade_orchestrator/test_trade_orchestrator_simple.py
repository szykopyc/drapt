...
from app.tests.pretty_prints_for_testing_purposes import terminalcolours, print_test_end_banner, print_test_start_banner
...
@pytest.mark.asyncio
async def test_trade_orchestrator_simple():
    async with test_engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)

    async with async_session() as session:
        print()
        print_test_start_banner(f"{__name__} Simple Trade Orchestrator Test")
        ...
        print(f"{terminalcolours.OKGREEN}Cash ledger after buy trade: {cash_ledger_after_buy}{terminalcolours.ENDC}\n")
        ...
        print(f"{terminalcolours.OKGREEN}Cash ledger after sell trade: {cash_ledger_after_sell}{terminalcolours.ENDC}\n")
        assert len(cash_ledger_after_sell) == 3  # Initial deposit + buy trade + sell trade
        assert cash_ledger_after_sell[2].amount == Decimal("120.000000")  # Sell trade proceeds
        ...
        assert final_balance["EUR"] == Decimal("1020.000000")  # 1000 - 100 (buy) + 120 (sell)
        ...
        assert aware_balance == Decimal("1020.000000")
