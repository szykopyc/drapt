...
export default function ForgotPassword() {
    ...
    const onSubmit = (data) => {
        if (
            ...
            getPasswordStrength(data.password) == "Weak"
        ) {
            return;
        }
        ...
        setModalData(data);
        if (userResetPasswordModalRef.current)
            userResetPasswordModalRef.current.showModal();
        resetForm();
    };
    ...
    return (
        <MainBlock>
            ...
            <ModalHelper
                ...
                reference={userResetPasswordModalRef}
                modalTitle={"Check your email"}
            >
                {modalData && (
                    <p>
                        If an account exists for{" "}
                        <span className="text-info">{modalData.email}</span>,
                        you'll receive an email with instructions to reset your
                        password.
                        <br />
                        Please check your inbox and spam folder, and wait 5
                        minutes before attempting to reset your password again.
                    </p>
                )}
            </ModalHelper>
        </MainBlock>
