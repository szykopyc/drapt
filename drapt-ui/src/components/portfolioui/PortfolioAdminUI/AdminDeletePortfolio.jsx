...
export default function AdminDeletePortfolio() {
  ...
  const [portfolioDeleteConfirmed, setPortfolioDeleteConfirmed] = useState(false);
  const [isPortfolioBeingDeleted, setIsPortfolioBeingDeleted] = useState(false);
  ...
  const guardedPortfolioDeleteHandler = async (portfoliodata) => {
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, 1000));
      queryClient.invalidateQueries(["portfolio", portfoliodata.portfolio_string_id]);
      navigate("/portfolio");
    }
  }
    ...
    <CustomCollapseArrow id={"deletePortfolio"} title={portfolioData ? `Delete ${portfolioData.name} Portfolio` : "Delete undefined portfolio"}>
      ...
      <div className="mt-3 flex flex-row justify-between gap-3">
        {!portfolioDeleteConfirmed ? (
          ...
          <>
            <CustomButtonInputStyle colour="success" onClick={() => guardedPortfolioDeleteHandler(portfolioData)}>
              {isPortfolioBeingDeleted ? "Deleting Portfolio...." : "Confirm Deletion"}
            </CustomButtonInputStyle>
            <CustomButtonInputStyle colour="neutral" onClick={() => setPortfolioDeleteConfirmed(false)}>
              Cancel
            </CustomButtonInputStyle>
          </>
        )}
