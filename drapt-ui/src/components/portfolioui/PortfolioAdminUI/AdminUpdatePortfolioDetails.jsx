...
export default function AdminUpdatePortfolioDetails() {
  ...
  const isPortfolioInitialCashAlreadySet = Number(portfolioOverviewData?.initial_cash) > 0.01;
  ...
  const isAnyUpdateFieldFilled = (watch("name") || watch("description")) || watch("currency") || watch("initial_cash");
  ...
  return (
    <CustomCollapseArrow title={"Manage Portfolio Details"} defaultOpen={false} onClose={() => reset()}>
      <form
        ...
        <div className="flex flex-col md:flex-row gap-3 justify-between">
          <div className="w-1/2">
            <FormField label="Set Currency">
              ...
              >
                <option value="" disabled>{isPortfolioInitialCashAlreadySet ? `Currency: ${currencyMapperDict[portfolioOverviewData?.currency]} - ${portfolioOverviewData?.currency}` : `Default ${currencyMapperDict[portfolioOverviewData?.currency]} - ${portfolioOverviewData?.currency}`}</option>
                {Object.entries(currencyMapperDict).map(([currencyStr, currencySymbol]) => (
                  <option key={currencyStr} value={currencyStr}>{currencySymbol} - {currencyStr}</option>
                ))}
              </select>
            </FormField>
          </div>
          <div className="w-1/2">
            <FormField label="Set Initial Cash">
              <input
                ...
                step="10"
                className="input input-bordered w-full"
                placeholder={isPortfolioInitialCashAlreadySet ? portfolioOverviewData?.initial_cash : "0.00"}
                disabled={isPortfolioInitialCashAlreadySet}
                {...register("initial_cash", {
                  validate: (value) => {
                    // Handle empty or 0-like inputs gracefully
                    const parsed = parseFloat(value);
                    if (isNaN(parsed) && watch("initial_cash")) return "Initial cash must be a valid number";
                    if (parsed < 0) return "Initial cash cannot be negative";
                    return true;
                  }
                })}
              />
